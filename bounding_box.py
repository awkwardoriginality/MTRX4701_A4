#!/usr/bin/env python3
"""
UR5e Workspace Bounding Box Setup
==================================
...
class BoundingBoxSetup(Node):
	def __init__(self):
		super().__init__('bounding_box_setup')
		self.pub = self.create_publisher(
			PlanningScene, '/planning_scene', 10
		)
		self.timer = self.create_timer(1.0, self.publish_scene)
		self.get_logger().info('BoundingBoxSetup node started - publishing in 1 s...')
	...
	def publish_scene(self):
		...
		scene.world.collision_objects.append(self.make_box(
			name='floor',
			size_xyz=(X_SPAN + 2 * thickness, Y_SPAN + 2 * thickness, thickness),
			pos_xyz=(X_CENTRE, Y_CENTRE, -thickness / 2.0)
		))
		...
		scene.world.collision_objects.append(self.make_box(
			name='ceiling',
			size_xyz=(X_SPAN + 2 * thickness, Y_SPAN + 2 * thickness, thickness),
			pos_xyz=(X_CENTRE, Y_CENTRE, CEILING_HT + thickness / 2.0)
		))

		# Front wall (+X, 100 cm - end effector direction)
		scene.world.collision_objects.append(self.make_box(
			name='front_wall',
			size_xyz=(thickness, Y_SPAN + 2 * thickness, Z_SPAN + 2 * thickness),
			pos_xyz=(FRONT_DIST + thickness / 2.0, Y_CENTRE, Z_CENTRE)
		))
		...
		scene.world.collision_objects.append(self.make_box(
			name='back_wall',
			size_xyz=(thickness, Y_SPAN + 2 * thickness, Z_SPAN + 2 * thickness),
			pos_xyz=(-BACK_DIST - thickness / 2.0, Y_CENTRE, Z_CENTRE)
		))

		# Right wall (+Y, 100 cm - right of end effector)
		scene.world.collision_objects.append(self.make_box(
			name='right_wall',
			size_xyz=(X_SPAN + 2 * thickness, thickness, Z_SPAN + 2 * thickness),
			pos_xyz=(X_CENTRE, RIGHT_DIST + thickness / 2.0, Z_CENTRE)
		))
		...
		scene.world.collision_objects.append(self.make_box(
			name='left_wall',
			size_xyz=(X_SPAN + 2 * thickness, thickness, Z_SPAN + 2 * thickness),
			pos_xyz=(X_CENTRE, -LEFT_DIST - thickness / 2.0, Z_CENTRE)
		))
