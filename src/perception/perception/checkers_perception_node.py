#!/usr/bin/env python3
"""Standalone perception package adapted to the canonical `checkers_bot` protocol."""
...
from checkers_bot.protocol import BoardStateReport


class CheckersPerceptionNode(Node):
    """Detect the board from camera frames and publish canonical game-state topics."""
    ...
    def __init__(self):
        ...
        self.declare_parameter("top_row_is_white", True)
        ...
        self.declare_parameter("king_height_threshold_mm", 9.0)
        self.declare_parameter("green_man_value", BLACK_MAN)
        self.declare_parameter("green_king_value", BLACK_KING)
        self.declare_parameter("purple_man_value", WHITE_MAN)
        self.declare_parameter("purple_king_value", WHITE_KING)

        # Output topics
        self.declare_parameter("board_state_topic", "/checkers/board_state")
        self.declare_parameter("board_state_report_topic", "/checkers/board_state_report")
        ...
        self.top_row_is_white = bool(self.get_parameter("top_row_is_white").value)
        ...
        self.king_height_threshold_mm = float(self.get_parameter("king_height_threshold_mm").value)
        self.green_man_value = int(self.get_parameter("green_man_value").value)
        self.green_king_value = int(self.get_parameter("green_king_value").value)
        self.purple_man_value = int(self.get_parameter("purple_man_value").value)
        self.purple_king_value = int(self.get_parameter("purple_king_value").value)

        self.board_state_topic = self.get_parameter("board_state_topic").value
        self.board_state_report_topic = self.get_parameter("board_state_report_topic").value
        ...
        self.board_pub = self.create_publisher(UInt8MultiArray, self.board_state_topic, 10)
        self.report_pub = self.create_publisher(String, self.board_state_report_topic, 10)
        ...
        self.get_logger().info("Integrated checkers perception node started")
    ...
    def get_aruco_dictionary(self, name):
        ...
        if name not in aruco_map:
            self.get_logger().warn(f"Unknown aruco_dictionary '{name}', using DICT_4X4_50")
    ...
    def bag_callback(self):
        try:
            ...
            self.get_logger().warn(f"Bag playback issue: {exc}")

    def image_callback(self, msg):
        frame = self.bridge.imgmsg_to_cv2(msg, desired_encoding="bgr8")
        self.last_rgb = frame
        self.process_frame(self.last_rgb, self.last_depth)
    ...
    def process_frame(self, frame, depth_frame):
        ...
        if warped is None:
            ...
            self.publish_report([0] * 64, board_blocked=True, stable_count=0, confidence=0.0)
            return

        warped_depth = None
        if depth_frame is not None:
            warped_depth, _ = self.get_warped_board(depth_frame, annotate=False)
        ...
        board, debug_img = self.classify_board(warped, warped_depth)
        flat64 = self.board_to_flat64(board)

        cv2.drawChessboardCorners(debug_img, self.chessboard_pattern_size, detected_corners, True)
        cv2.putText(
            debug_img,
        ...
        if self.last_board is not None and flat64 == self.last_board:
            self.stable_count += 1
        else:
            self.stable_count = 1
            self.last_board = flat64[:]

        self.publish_board(flat64)
        confidence = 1.0 if any(flat64) else 0.6
        self.publish_report(flat64, board_blocked=False, stable_count=self.stable_count, confidence=confidence)

        if self.stable_count >= self.stable_required_frames:
            if self.last_published_board is None or flat64 != self.last_published_board:
                self.last_published_board = flat64[:]
                self.get_logger().info(f"Published canonical board with stability={self.stable_count}")
    ...
    def check_chessboard_visible(self, warped):
        ...
        if hasattr(cv2, "findChessboardCornersSB"):
            ...
            found, corners = cv2.findChessboardCornersSB(gray, self.chessboard_pattern_size, flags_sb)

        if not found:
            flags = cv2.CALIB_CB_ADAPTIVE_THRESH | cv2.CALIB_CB_NORMALIZE_IMAGE | cv2.CALIB_CB_FILTER_QUADS
            found, corners = cv2.findChessboardCorners(gray, self.chessboard_pattern_size, flags)
            if found:
        ...
        if found and corners is not None and len(corners) == expected_corners:
            cv2.drawChessboardCorners(debug, self.chessboard_pattern_size, corners, found)
    ...
    def get_warped_board(self, frame, annotate=True):
        h, w = frame.shape[:2]
        ...
        roi_big = cv2.resize(roi, None, fx=self.zoom_scale, fy=self.zoom_scale, interpolation=cv2.INTER_LINEAR)
        outline_img = roi_big.copy() if annotate else None

        gray = roi_big if len(roi_big.shape) == 2 else cv2.cvtColor(roi_big, cv2.COLOR_BGR2GRAY)
        ...
        if ids is not None:
            ...
            if all(tag_id in ids_flat for tag_id in required_ids):
                ...
                for corner, tag_id in zip(corners, ids_flat):
                    ...
                    if int(tag_id) in required_ids:
                        tag_corners[int(tag_id)] = pts
                        tag_centres[int(tag_id)] = np.mean(pts, axis=0)
                ...
                if self.last_src_pts is None:
                    ...
                    src_pts = self.src_alpha * self.last_src_pts + (1.0 - self.src_alpha) * src_pts_new
                    layout_centre = self.src_alpha * self.last_layout_centre + (1.0 - self.src_alpha) * layout_centre_new

                self.last_src_pts = src_pts.copy()
                self.last_layout_centre = layout_centre.copy()
                self.missing_aruco_count = 0
                detection_ok = True
                if annotate and outline_img is not None:
        ...
        if not detection_ok:
            if (
                ...
                if annotate and outline_img is not None:
                    cv2.putText(outline_img, "ARUCO LOST - NO CACHE", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)
                return None, outline_img

        dst_pts = np.array(
            [[0, 0], [self.output_size - 1, 0], [self.output_size - 1, self.output_size - 1], [0, self.output_size - 1]],
            dtype=np.float32,
        )

        if annotate and outline_img is not None:
            outline_pts = src_pts.astype(np.int32).reshape((-1, 1, 2))
            outline_colour = (0, 255, 255) if using_cache else (0, 255, 0)
            text = (
                f"USING CACHED ARUCO {self.missing_aruco_count}/{self.max_missing_aruco_frames}"
                if using_cache else "LIVE ARUCO DETECTION"
            )
            ...
            cv2.circle(outline_img, tuple(layout_centre.astype(np.int32)), 8, (255, 0, 0), -1)
            cv2.putText(outline_img, text, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, outline_colour, 2)

        H = cv2.getPerspectiveTransform(src_pts, dst_pts)
        warped = cv2.warpPerspective(roi_big, H, (self.output_size, self.output_size))
        return warped, outline_img

    def classify_board(self, warped, warped_depth):
        ...
        for row in range(8):
            for col in range(8):
                ...
                roi_hsv = hsv[y1 + margin:y2 - margin, x1 + margin:x2 - margin]
                roi_bgr = warped[y1 + margin:y2 - margin, x1 + margin:x2 - margin]
                roi_depth = None
                if warped_depth is not None:
                    roi_depth = warped_depth[y1 + margin:y2 - margin, x1 + margin:x2 - margin]

                piece_value, label = self.classify_square(roi_hsv, roi_bgr, roi_depth)
                board[row, col] = piece_value

                cv2.rectangle(debug_img, (x1, y1), (x2, y2), (255, 255, 255), 1)
                cv2.putText(debug_img, label, (x1 + 20, y1 + 60), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 2)

        return board, debug_img

    def classify_square(self, roi_hsv, roi_bgr, roi_depth):
        if roi_hsv.size == 0:
            return 0, "."

        green_mask = cv2.inRange(roi_hsv, self.green_lower, self.green_upper)
        purple_mask = cv2.inRange(roi_hsv, self.purple_lower, self.purple_upper)

        crop_area = roi_hsv.shape[0] * roi_hsv.shape[1]
        green_ratio = cv2.countNonZero(green_mask) / crop_area
        purple_ratio = cv2.countNonZero(purple_mask) / crop_area

        if green_ratio > self.piece_min_ratio and green_ratio > purple_ratio:
            is_king = self.detect_king(roi_bgr, roi_depth, green_mask)
            return (self.green_king_value if is_king else self.green_man_value), ("GK" if is_king else "G")

        if purple_ratio > self.piece_min_ratio and purple_ratio > green_ratio:
            is_king = self.detect_king(roi_bgr, roi_depth, purple_mask)
            return (self.purple_king_value if is_king else self.purple_man_value), ("PK" if is_king else "P")

        return 0, "."

    def detect_king(self, roi_bgr, roi_depth, piece_mask):
        if roi_depth is not None:
            valid_depths = roi_depth[piece_mask > 0]
            if len(valid_depths) > 0:
                return float(np.mean(valid_depths)) > self.king_height_threshold_mm
        ...
        circles = cv2.HoughCircles(
            ...
            param2=30,
            minRadius=5,
            maxRadius=max(6, self.cell_size // 3),
        )
        return circles is not None and len(circles[0]) > 1

    def board_to_flat64(self, board):
        flat64 = [0] * 64
        for row in range(8):
            for col in range(8):
                canonical_row = (7 - row) if self.top_row_is_white else row
                flat64[canonical_row * 8 + col] = int(board[row, col])
        return flat64

    def publish_board(self, flat64):
        msg = UInt8MultiArray()
        msg.data = flat64
        self.board_pub.publish(msg)

    def publish_report(self, flat64, *, board_blocked, stable_count, confidence):
        report = String()
